...
External import operations compose parser, normalizer, deduplication, PDF sync, ingest, summary-mode selection, and lint.
...
- **AI summary**: Claude reads the PDF/text and writes a scholarly source page summary with methods, findings, limitations, and relevance to the project. Use for important papers, review papers, and papers likely to affect synthesis.
- **Automatic summary**: run `python scripts/rag/summarize_sources.py --rag-dir RAG` for metadata/PDF-text-backed placeholder filling. Use only as a fast fallback for low-priority papers.
...
7. Run `ingest` to create source pages for new entries.
8. Ask the user to choose AI summary / automatic summary / skip for the newly imported papers.
...
6. Run `ingest` to create source pages for new entries.
7. Ask the user to choose AI summary / automatic summary / skip for the imported papers.
8. Apply the selected summary mode and run lint.
...
1. Use Claude to refine the user's natural-language description into one or more search queries.
